...
export default function MenuDisplay({ online_user }) {
  ...
  return (
    <>
      <div onClick={() => setIsDisplay(true) } className="absolute md:hidden z-10 right-6 top-5">
      ...
      {setIsDisplay && (
        <>
          <button onClick={() => setIsDisplay(false)} type="button" className={`${isDisplay ? 'md:hidden z-10 block fixed inset-0 -pt-20 w-full h-full overflow-hidden cursor-default bg-black bg-opacity-60 focus:outline-none' : 'hidden'}`}></button>
          <div className={`md:hidden z-10 w-full ${isDisplay ? 'fixed' : 'hidden'}`}>
            <div className="md:hidden fixed left-0 top-0 flex flex-col w-full max-w-[18rem] h-full overflow-hidden space-y-5 rounded-none md:rounded-l-2xl bg-modern-dim border-r border-modern-white border-opacity-10">
              <div className="flex flex-col items-center w-full h-full overflow-y-auto overflow-x-hidden transition ease-in-out duration-300 bg-gray-100 dark:bg-[#262B37]">
                <div className="flex flex-col items-center w-full h-full px-3 py-5 space-y-5">
                  <div className="flex flex-col items-center w-full">
                    <span className="font-bold text-xl md:text-2xl text-[#62A9FF]">VEO PORTAL</span>
                    <span className="font-light text-sm">My daily blogs</span>
                  </div>
                  {!online_user && (
                    <div className="flex flex-col w-full pt-3 px-3 space-y-3">
                  ...
                  {online_user && (
                    <div className="flex flex-col w-full pt-3 space-y-3 pb-20">
                      <div className="flex flex-col md:flex-row items-center space-x-0 md:space-x-5">
